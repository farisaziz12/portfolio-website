...
export default function GeneralSubscribe({ compact = false }: Props) {
  ...
  if (status === 'success') {
    return (
      <div className={compact ? 'text-center' : 'rounded-xl border border-[var(--color-edge)] bg-[var(--color-surface-raised)] p-8 text-center'}>
        <p className="text-lg font-display font-semibold text-[var(--color-ink)]">
          You're in! 🎉
        </p>
        <p className="text-sm text-[var(--color-ink-muted)] mt-2">
          I'll let you know when I'm speaking near you.
        </p>
      </div>
    );
  }
  ...
  if (compact) {
    return (
      <form onSubmit={handleSubmit} className="space-y-3">
        <p className="text-sm font-medium text-[var(--color-ink-muted)]">Follow my conference schedule</p>
  ...
  return (
    <div className="rounded-xl border border-[var(--color-edge)] bg-[var(--color-surface-raised)] p-8">
      <h3 className="text-lg font-display font-semibold text-[var(--color-ink)] mb-1">
        Follow my conference schedule
      </h3>
      <p className="text-sm text-[var(--color-ink-muted)] mb-6">
        Find out when I'm speaking near you.
      </p>
